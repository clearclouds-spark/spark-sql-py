...
def get_http_server(time, rdd):
    try:
        ...
        urls = sqlContext.sql("""SELECT dst_group_id, dst_ipv4, 
            ...
            count(*) as requests 
            FROM http where url is not null 
            group by dst_group_id, dst_ipv4""")   
        urls.registerAsTable("urls")
        output = {}
        for opt in ['bytes_total', 'bytes_in', 'bytes_out', 'latency', 'requests']:
            output[opt] = {}
            for group_id in total_groups:
                topN_sql = """select dst_ipv4 as ip, bytes_total, bytes_in, bytes_out, latency, requests 
...
if __name__ == "__main__":
    ...
    kafka_spark_streaming_sql_main("HttpServer", brokers, topic, 5, get_http_server)
