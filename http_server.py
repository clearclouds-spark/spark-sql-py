...
if __name__ == "__main__":
    ...
    urls = sqlContext.sql("SELECT dst_group_id, dst_ipv4, sum(in_bytes + out_bytes) as total_bytes, sum(in_bytes) as in_bytes, sum(out_bytes) as out_bytes, sum(latency_sec * 1000000 + latency_usec) as latency, count(*) as requests FROM http where url is not null group by dst_group_id, dst_ipv4")
    ...
    for opt in ['total_bytes', 'in_bytes', 'out_bytes', 'latency', 'requests']:
        ...
        for group_id in total_groups:
            topN_sql = "select dst_ipv4, total_bytes, in_bytes, out_bytes, latency, requests from urls where dst_group_id = %s order by %s desc limit 50" % (group_id.dst_group_id, opt)
    ...
    with open("http_server_result.json", "w") as f:
